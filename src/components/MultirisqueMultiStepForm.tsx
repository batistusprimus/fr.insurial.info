...
interface MultirisqueLeadData {
  ...
  premisesType: string; // owned/rented/coworking
  premisesSize: string;
  valueOfEquipment: string;
  hasInventory: string; // yes/no
  inventoryValue?: string;

  // Step 3 — Activité & risques
  operatingHours: string;
  receivesPublic: string; // yes/no
  hasHighValueEquipment: string; // yes/no (serveurs, machines)
  hasDangerousActivities: string; // yes/no

  // Step 4 — Couverture actuelle
  hasCurrentMultirisque: string; // yes/no
  currentProvider?: string;
  wantsLossOfEarnings: string; // yes/no
...
const initialData: MultirisqueLeadData = {
  companySize: '',
  industry: '',
  location: 'FR',
  annualRevenue: '',
  ...
  operatingHours: '',
  receivesPublic: '',
  hasHighValueEquipment: '',
  hasDangerousActivities: '',

  hasCurrentMultirisque: '',
  currentProvider: '',
  wantsLossOfEarnings: '',
  priorClaims: '',
  claimDetails: '',
...
export default function MultirisqueMultiStepForm() {
  const [currentStep, setCurrentStep] = useState(1);
  const [form, setForm] = useState<MultirisqueLeadData>(initialData);
  ...
  const update = (field: keyof MultirisqueLeadData, value: string | boolean) => {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      ...
      if (form.hasPhysicalPremises === 'yes') {
        if (!form.premisesType) e.premisesType = 'Requis';
        if (!form.premisesSize) e.premisesSize = 'Requis';
        if (!form.valueOfEquipment) e.valueOfEquipment = 'Requis';
      }
      if (!form.hasInventory) e.hasInventory = 'Requis';
    }
    if (step === 3) {
      if (!form.operatingHours) e.operatingHours = 'Requis';
      if (!form.receivesPublic) e.receivesPublic = 'Requis';
      if (!form.hasHighValueEquipment) e.hasHighValueEquipment = 'Requis';
      if (!form.hasDangerousActivities) e.hasDangerousActivities = 'Requis';
    }
    if (step === 4) {
      if (!form.hasCurrentMultirisque) e.hasCurrentMultirisque = 'Requis';
      if (!form.wantsLossOfEarnings) e.wantsLossOfEarnings = 'Requis';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...form,
        coverageType: ['Multirisque Professionnelle'],
        source: 'fr.insurial.info/multirisque-professionnelle',
        ...utm,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border p-6 md:p-8">
      ...
      {currentStep === 2 && (
        <div className="space-y-6">
          <h2 className="text-2xl font-archivo-black text-gray-900">Vos locaux et biens</h2>
          <div>
          ...
          {form.hasPhysicalPremises === 'yes' && (
            <>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Type de locaux *</label>
                <select
                  value={form.premisesType}
                  onChange={e => update('premisesType', e.target.value)}
                  className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
                >
                  <option value="">Sélectionner</option>
                  <option value="owned">Propriétaire</option>
                  <option value="rented">Locataire</option>
                  <option value="coworking">Coworking / Bureaux partagés</option>
                </select>
                {errors.premisesType && <p className="text-red-600 text-sm mt-1">{errors.premisesType}</p>}
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Surface approximative *</label>
                <select
                  value={form.premisesSize}
                  onChange={e => update('premisesSize', e.target.value)}
                  className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
                >
                  <option value="">Sélectionner</option>
                  <option value="<50m2">Moins de 50 m²</option>
                  <option value="50-200m2">50 - 200 m²</option>
                  <option value="200-500m2">200 - 500 m²</option>
                  <option value=">500m2">Plus de 500 m²</option>
                </select>
                {errors.premisesSize && <p className="text-red-600 text-sm mt-1">{errors.premisesSize}</p>}
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Valeur du matériel/équipements *</label>
                <select
                  value={form.valueOfEquipment}
                  onChange={e => update('valueOfEquipment', e.target.value)}
                  className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
                >
                  <option value="">Sélectionner</option>
                  <option value="<20K">Moins de 20 K€</option>
                  <option value="20K-50K">20 K€ - 50 K€</option>
                  <option value="50K-200K">50 K€ - 200 K€</option>
                  <option value=">200K">Plus de 200 K€</option>
                </select>
                {errors.valueOfEquipment && <p className="text-red-600 text-sm mt-1">{errors.valueOfEquipment}</p>}
              </div>
            </>
          )}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Stocks / marchandises ? *</label>
            <select
              value={form.hasInventory}
              onChange={e => update('hasInventory', e.target.value)}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
            >
              <option value="">Sélectionner</option>
              <option value="yes">Oui</option>
            ...
            {errors.hasInventory && <p className="text-red-600 text-sm mt-1">{errors.hasInventory}</p>}
          </div>
          {form.hasInventory === 'yes' && (
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Valeur estimée du stock (optionnel)</label>
              <input
                type="text"
      ...
      {currentStep === 3 && (
        <div className="space-y-6">
          <h2 className="text-2xl font-archivo-black text-gray-900">Activité et exposition</h2>
          <div>
            ...
            >
              <option value="">Sélectionner</option>
              <option value="standard">Horaires de bureau (9h-18h)</option>
              <option value="extended">Horaires étendus (jusqu\'à 22h)</option>
              <option value="24/7">24h/24, 7j/7</option>
            </select>
            {errors.operatingHours && <p className="text-red-600 text-sm mt-1">{errors.operatingHours}</p>}
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Recevez-vous du public ? *</label>
            <select
              value={form.receivesPublic}
              onChange={e => update('receivesPublic', e.target.value)}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
            >
              <option value="">Sélectionner</option>
              <option value="yes">Oui</option>
            ...
            {errors.receivesPublic && <p className="text-red-600 text-sm mt-1">{errors.receivesPublic}</p>}
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Équipements sensibles (serveurs, machines coûteuses) ? *</label>
            <select
              value={form.hasHighValueEquipment}
              onChange={e => update('hasHighValueEquipment', e.target.value)}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
            >
              <option value="">Sélectionner</option>
              <option value="yes">Oui</option>
            ...
            {errors.hasHighValueEquipment && <p className="text-red-600 text-sm mt-1">{errors.hasHighValueEquipment}</p>}
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Activités à risque (produits dangereux, machines) ? *</label>
            <select
              value={form.hasDangerousActivities}
              onChange={e => update('hasDangerousActivities', e.target.value)}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
            >
              <option value="">Sélectionner</option>
              <option value="yes">Oui</option>
            ...
            {errors.hasDangerousActivities && <p className="text-red-600 text-sm mt-1">{errors.hasDangerousActivities}</p>}
          </div>
        </div>
      )}
      ...
      {currentStep === 4 && (
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Multirisque actuelle ? *</label>
            <select
              value={form.hasCurrentMultirisque}
              onChange={e => update('hasCurrentMultirisque', e.target.value)}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
            >
              <option value="">Sélectionner</option>
              <option value="yes">Oui</option>
            ...
            {errors.hasCurrentMultirisque && <p className="text-red-600 text-sm mt-1">{errors.hasCurrentMultirisque}</p>}
          </div>
          {form.hasCurrentMultirisque === 'yes' && (
            <div>
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Souhaitez-vous une garantie perte d\'exploitation ? *</label>
            <select
              value={form.wantsLossOfEarnings}
              onChange={e => update('wantsLossOfEarnings', e.target.value)}
              className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
            >
              <option value="">Sélectionner</option>
              <option value="yes">Oui</option>
            ...
            {errors.wantsLossOfEarnings && <p className="text-red-600 text-sm mt-1">{errors.wantsLossOfEarnings}</p>}
          </div>
          <div>
          ...
          {form.priorClaims === 'yes' && (
            <div>
              ...
              <textarea
                ...
                placeholder="Dégât des eaux, incendie, vol..."
                rows={3}
                className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent text-gray-900"
              />
            </div>
